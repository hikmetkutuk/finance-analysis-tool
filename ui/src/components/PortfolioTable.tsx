...
import type { SortDir, SortKey, Stock } from '../types'
...
function price(v: number | null): React.ReactNode {
  if (v === null || v === undefined) return <span className="text-slate-600">—</span>
  return <span className="tabular-nums text-slate-200">${v.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
}
...
function SignalBadge({ label, score }: Readonly<{ label: string | null; score: number | null }>) {
...
const COLUMNS: Col[] = [
  { key: 'Kod', label: 'Ticker', align: 'left' },
  { key: 'Sektör', label: 'Sektör', align: 'left' },
  { key: 'Güncel Fiyat', label: 'Fiyat', align: 'right', fmt: (v) => price(v as number | null) },
  { key: 'Ortalama Adil Fiyat', label: 'Adil Fiyat', align: 'right', fmt: (v) => price(v as number | null) },
  { key: 'Beklenen Getiri (%)', label: 'Upside', align: 'right', fmt: (v) => pct(v as number | null) },
  { key: 'Sinyal Güven Seviyesi', label: 'Sinyal', align: 'left', fmt: (v, row) => <SignalBadge label={v as string | null} score={row['Sinyal Kalite Skoru'] as number | null} /> },
  { key: 'WACC', label: 'WACC', align: 'right', fmt: (v) => num(v as number | null) },
  { key: 'DCF Değerlemesi', label: 'DCF', align: 'right', fmt: (v) => price(v as number | null) },
  { key: 'F/K Değerlemesi', label: 'P/E', align: 'right', fmt: (v) => price(v as number | null) },
  { key: 'EV/EBITDA Değerlemesi', label: 'EV/EBITDA', align: 'right', fmt: (v) => price(v as number | null) },
]
...
export default function PortfolioTable({ stocks, selected, onSelect }: Readonly<Props>) {
    ...
    <div className="rounded-xl border border-slate-800 bg-slate-900/50 overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-800">
        <input
          ...
          placeholder="Ticker veya sektör ara…"
